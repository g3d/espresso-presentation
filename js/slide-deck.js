...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (settings.title) {
    document.title = settings.title.replace(/<br\/?>/, ' ') + ' - Lviv RUG';
  ...
  if (this.config_.presenters) {
    ...
    if (presenters.length == 1) {
      ...
      var irb = p.irb ? '<span>irb</span>' +
          '<a href="http://irc.freenode.net/' + p.irb + '">' +
          p.irb + '</a>' : '';
      ...
      var html2 = [gplus, www, twitter, email, irb, github].join('<br>');
